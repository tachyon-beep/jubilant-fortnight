"""Scheduler utilities for Gazette ticks (infrastructure layer)."""
...
from great_work.analytics import write_calibration_snapshot
...
class GazetteScheduler:
    ...
    def _publish_to_container(self, archive_path: Path) -> None:
        ...
        try:
            ...
            _get_telemetry().track_system_event(
    ...
    def _prune_snapshots(self, snapshots_dir: Path) -> None:
        ...
        for stale in snapshots[self._archive_snapshot_limit :]:
            try:
                ...
                logger.debug("Failed to remove stale snapshot: %s", stale)

    def _publish_to_pages(self, archive_path: Path) -> None:
        """Publish archive to GitHub Pages-compatible directory, if configured."""

        if not self._archive_pages_enabled or not self._archive_pages_dir:
            return
        try:
            target_root = self._archive_pages_dir
            subdir = self._archive_pages_subdir.strip("/ ")
            target = target_root / subdir if subdir else target_root
            source = archive_path.resolve()
            ...
            if self._archive_pages_nojekyll:
                (target_root / ".nojekyll").write_text("")
            _get_telemetry().track_system_event(
                "archive_published_pages",
                source="gazette_scheduler",
            )
            logger.info("Archive published to pages directory: %s", target)
        except Exception as exc:
            logger.exception("Failed to publish archive to pages directory")
            try:
                _get_telemetry().track_system_event(
...
def _get_telemetry():
    """Defer telemetry accessor to the compat module for monkeypatching.

    Tests patch `great_work.scheduler.get_telemetry`; by resolving through that
    module we preserve the patch point even though the implementation lives here.
    """

    from great_work.scheduler import get_telemetry as _compat_get_telemetry  # type: ignore

    return _compat_get_telemetry()


def _build_scheduler():
    """Construct a scheduler using the compat import for patchability in tests."""

    from great_work.scheduler import BackgroundScheduler  # type: ignore
