...
def recommend_settings(db_path: Path, days: int = 30) -> Dict[str, float]:
    ...
    outstanding = [entry["debt"] for entry in latest_per_player.values() if entry["debt"] > 0]
    thresholds = [entry["threshold"] for entry in latest_per_player.values() if entry["threshold"] > 0]
    days_remaining = [entry["days_remaining"] for entry in latest_per_player.values() if entry["days_remaining"] > 0]
    ...
    median_days_remaining = statistics.median(days_remaining) if days_remaining else 14.0

    suggested_base_cost = max(1.0, round(median_debt / 4.0, 1)) if median_debt else 3.0
    suggested_reprisal_threshold = max(2.0, round((median_debt / max(suggested_base_cost, 1.0)) + 1.0)) if median_debt else median_threshold
...
def main() -> None:
    ...
    print(f"Players sampled: {recommendations['players_sampled']} (window {recommendations['window_days']}d)")
    ...
    print(f"seasonal_commitments.reprisal_threshold ≈ {recommendations['suggested_reprisal_threshold']}")
