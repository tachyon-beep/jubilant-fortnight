...
def _seed_legacy_followup(
    state: GameState, *, scholar_id: str, kind: str, resolve_at: str
) -> None:
...
def test_preview_and_migrate_followups(tmp_path) -> None:
    ...
    _seed_legacy_followup(
        state, scholar_id="s-1", kind="defection_grudge", resolve_at=now_iso
    )
    ...
    orders = [
        order for order in state.list_orders(order_type="followup:defection_grudge")
    ]
    assert len(orders) == 1
...
def test_manage_orders_summary(tmp_path, capsys) -> None:
    ...
    args = parser.parse_args(
        ["--state-db", str(state._db_path), "summary", "--json"]
